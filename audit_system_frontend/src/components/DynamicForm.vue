...
interface Field {
    ...
    is_required: boolean;
    field_order: number;
    options_json: FieldOption[] | null;
}
interface Section { id: number; section_name: string; section_order: number; fields: Field[]; }
interface FormDef { id: number; code: string; name: string; sections: Section[]; }
...
async function loadEverything() {
    ...
    try {
        ...
        const matched = forms.find((f: any) => f.code === code);
        if (!matched) {
            errorMsg.value = `Form dengan kode ${code} tidak ditemukan di database (tabel audit_forms).`;
            return;
        }
        form.value = matched;
        ...
        const matchedEng = engagements.find((e: any) => e.client?.id === selectedCompany.id);
        if (!matchedEng) {
        ...
        console.error(e);
        errorMsg.value = 'Terjadi kesalahan saat memuat form.';
    } finally {
        loading.value = false;
    }
}
...
<template>
    <DashboardLayout>
        ...
        <div class="dynamic-form" v-else-if="form">
            ...
            <div v-if="!form.sections || form.sections.length === 0" class="card empty-state">
            ...
            <footer class="action-footer card">
                <template v-if="canEdit">
                    <div class="action-right">
                        <button class="btn secondary" :disabled="saving" @click="handleSaveDraft">Simpan Draft</button>
                        <button class="btn primary" :disabled="saving" @click="handleSubmit">Submit untuk Review</button>
                    </div>
                </template>
                ...
                <template v-else-if="canReview">
                    ...
                    <div class="action-right">
                        <button class="btn danger" @click="handleReview('request_revision')">Minta Revisi</button>
                        <button class="btn primary" @click="handleReview('approve')">Setujui &amp; Teruskan ke Partner</button>
                    </div>
                </template>
                ...
                <template v-else-if="canApprove">
                    ...
                    <div class="action-right">
                        <button class="btn danger" @click="handleApprove('reject')">Tolak / Revisi</button>
                        <button class="btn success" @click="handleApprove('approve')">Approve Final</button>
                    </div>
                </template>
...
.action-footer { display: flex; flex-direction: column; gap: 0.75rem; }
.action-right { display: flex; justify-content: flex-end; gap: 0.75rem; }
.comment-input { min-height: 60px; }
